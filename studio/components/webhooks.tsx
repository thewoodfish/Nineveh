...
export function Webhooks({ project }: { project: string }) {
  ...
  return (
    <Card>
      ...
      {error && (
        <p className="border-b border-zinc-200 px-4 py-2 text-sm text-red-600 dark:border-zinc-800">
          {error}
        </p>
      )}
      ...
      <ul className="divide-y divide-zinc-100 dark:divide-zinc-800">
        {hooks?.map((hook) => (
          <li key={hook.name} className="px-4 py-3">
            ...
            {hook.last_error && (
              <p className="mt-1.5 truncate text-xs text-red-600" title={hook.last_error}>
                last attempt: {hook.last_error}
              </p>
            )}

            <div className="mt-2 flex items-center gap-2">
              <code className="min-w-0 flex-1 truncate rounded bg-zinc-50 px-2 py-1 font-mono text-xs dark:bg-zinc-900">
...
function Health({ hook }: { hook: WebhookInfo }) {
  ...
  if (!hook.delivered) return <span className="text-xs text-zinc-400">nothing sent yet</span>;
  return <span className="text-xs text-emerald-600">delivered to {hook.delivered}</span>;
}
